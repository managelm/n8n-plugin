#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
# ManageLM n8n Plugin — Deploy script
#
# Tags, pushes clean project files to GitHub (no internal scripts),
# and creates a GitHub release with the tgz attached.
#
# Prerequisites:
#   - package.sh has been run (tgz exists)
...
GITHUB_REPO="managelm/n8n-plugin"
...
RELEASE_BODY="## ManageLM n8n Community Node ${VERSION}

### Install
\`\`\`bash
# From n8n community panel
Settings > Community Nodes > Install > ${PLUGIN_NAME}

# Or from CLI
cd ~/.n8n && npm install ${PLUGIN_NAME}@${VERSION}
\`\`\`

### Download
- \`${TARBALL}\` — npm package (installable with \`npm install ./${TARBALL}\`)

See [documentation](https://www.managelm.com/plugins/n8n.html) for full setup guide."
